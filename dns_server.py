...
class DNSHandler(BaseRequestHandler):
    def handle(self):
        ...
        if base_domain not in qn:
            ...
            print(
                f"ERROR: DNS {qt}:{qn} from {client}:{port} wrong sub domain format NXDOMAIN"
            )
            reply = DNSRecord(
            ...
            for key in config_A:
                if f".{key}.{base_domain}." in qn:
                    ...
                    try:
                        ...
                        found = True
                        print(
                            f"INFO: DNS {qt}:{qn} from {client}:{port} -> {ip_address}"
                        )
            ...
            if not found:
                ...
                print(
                    f"ERROR: DNS {qt}:{qn} from {client}:{port} wrong sub domain NXDOMAIN"
                )
        else:
